...
# (开局观察由框架 walker 自动处理,不需要 per-game extractor/applier)
...
| 症状 | 可能原因 | 修复 |
|------|---------|------|
| belief 第一步就发散 | schema 把某个 perspective 应当可见的字段声明为 hidden,或 base viz 没含 owner-only-first-axis | 对照 encoder 看 perspective 能看到什么,确保 schema 中那个 slot 的 base viz 给了观察者 viz=1(walker 才会把它 emit 进 initial obs) |
