...
## 4.7 响应式 UI 的尺寸要两个轴都用比例，不要混"百分比 + 固定像素"

**踩过的坑（Quoridor 手机端，同一 bug 的第三面）**：legal 放墙预览 pill 的 CSS 是：

```css
.edge-slot.edge-h::after {
  width: calc(100% - 8px);   /* 宽度按 slot 比例变 */
  height: 8px;               /* 但高度固定 8px */
}
```

桌面 slot=34px：pill 是 26×8，长宽比 3.2，看着是长条。
手机 slot=17px：pill 是 9×8，长宽比 1.1，看着是正方形/圆点。

**根因**：混用"宽度按 slot 比例 + 高度固定像素"，slot 缩小一倍时固定像素占比就翻倍，宽高比塌了。桌面上看对只是因为当时 slot 大。

**正确做法**：凡是要保持"视觉比例一致"的尺寸，**两个轴都用百分比**：
...
现在桌面和手机都是 2.83:1 的长条，比例稳定。

**判断规则**：响应式容器里子元素的尺寸，如果设计意图是"XX% 比例"(pill 形状、圆点、图标)，那两个轴都用 `%` 或 `em`；如果设计意图是"一个绝对厚度"(边框、分割线、阴影)，那两个轴都用 `px`。**不要一轴一种单位** — 父容器一缩小比例就坏。
...
### 5.2 隐藏信息游戏：UI 不能成为旁路通道

只要存在对人类玩家不可见的对手私密状态（手牌、身份牌等），Web 前端就有两个会泄漏隐藏信息的默认行为，必须显式关掉：

1. **胜率预估 / 掉分分析**：信息栏胜率读 MCTS 根节点 `root_values[humanPlayer]`，搜索是从**真实状态**（含对手隐藏手牌）出发的，胜率随对手实际拿到的牌剧烈摆动；玩家逆向就能推出对手的牌。掉分提示来自同一份 root values，所以同一个开关同时管控。隐藏信息游戏在 `createApp({ showWinrateDefault: false })`，玩家想看可在侧栏「高级功能 → 显示胜率预估」自行开启。
2. **替对手落子**：force 模式让人类替 AI 走一步——但人类看不到对手手牌，提交后引擎按真实手牌检查合法性，等于把"哪些动作合法"反馈回玩家。Love Letter / Coup 直接 `disableForce: true`。

**部分隐藏的混合做法**：Splendor 既有桌面明置的保留牌，也有从牌堆顶拿的暗保留牌。可以保留 force 模式（不设 `disableForce`），但在 `renderPlayerArea` 里按 `item.visible` 过滤——明保留可买，暗保留在 force 模式下不可点。粒度由游戏前端自己决定，不要走"全有或全无"。

### 5.3 侧栏 ops-msg：唯一的瞬态文字通道

侧栏「高级功能」上的 `#ops-msg` 区是**唯一**用来显示"刚刚发生了什么瞬态信息"的位置：开局后写 `config.gameIntro`（简短操作说明）、AI 第一次落子后自动清掉、悔棋后写"已悔棋"、点击替对手落子后写该流程的一句话提示，掉分提示等也走这里。

不要为了多显示一条状态而在游戏区或玩家区临时塞一行文字——这违反 §1 空间锚定，并且和现有的 ops-msg 形成竞态（哪条覆盖哪条说不清）。需要持久显示的信息走 `extensions` 进信息栏 pill；需要瞬态提示的统一走 `sidebar.setOpsMsg(...)`。

新开局时框架会同步调 `infoPanel.reset()` 清掉上一局残留 pill；不要在游戏自己的 `onGameStart` 里再去操作信息栏 DOM。
...
- [ ] 要保持比例一致的子元素两个轴都用 `%`（或都用 `px`），不要一轴百分比一轴固定像素——手机上 slot 缩小时比例会塌（§4.7）
- [ ] **隐藏信息游戏**显式设 `disableForce: true` + `showWinrateDefault: false`——胜率与 force 模式都会从 AI 那边把对手私密信息泄回给玩家（§5.2）
- [ ] 部分隐藏（如 Splendor 暗保留 vs 明保留）通过 `renderPlayerArea` 按 `item.visible` 粒度限制 force 模式可点项，不要整体禁用
- [ ] 操作类瞬态文字一律走 `sidebar.setOpsMsg(...)`，不要在游戏区临时塞文字行（§5.3）；持久信息走侧栏 `extensions` pill
