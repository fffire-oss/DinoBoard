...
- **悔棋**：回退玩家最后一步 + AI 回应
- **替对手落子**：玩家替 AI 选动作，用于调试。隐藏信息游戏可在 `createApp({ disableForce: true })` 整体关闭（人类看不到对手手牌，无法替其选动作）
- **智能提示**：AI 推荐最佳动作和胜率，不落子
- **显示胜率预估**（侧栏勾选）：控制信息栏胜率 pill 与"失误/严重失误"标记。两者共用 MCTS root values（搜索从含真实隐藏状态出发），所以隐藏信息游戏须 `showWinrateDefault: false`，玩家想看可手动开启
- **对局中显示录像栏**（侧栏勾选）：是否始终保留录像窗口（默认 true）

开局时侧栏 ops-msg 写入 `config.gameIntro` 作为操作提示，AI 第一次落子时自动清掉，让位于"已悔棋"等瞬态消息；新开局会同步清理上一局残留信息栏。详见 [Guide §13.5.1 + §14.2](GAME_DEVELOPMENT_GUIDE.md#1351-隐藏信息游戏的两个必备开关)。
